...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-dubxcoin/build/test/createRandomStateTest\" dubxcoin/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyethereum
	OUTPUT_PYTHON="$(docker run --rm dubxcoin/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm dubxcoin/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm dubxcoin/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?

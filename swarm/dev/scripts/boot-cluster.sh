...
#
# The cluster contains a bootnode, a gdubx node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gdubx and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gdubx node with --datadir pointing at <base-dir>/gdubx
...
start_geth_node() {
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gdubx account address"
  fi
...
create_account() {
  ...
  gdubx --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}

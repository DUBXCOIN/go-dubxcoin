...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", gdubx.LightScryptN, gdubx.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.dubxcoin", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gdubx-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/dubxcoin/
gethtest", "libs"} {
	...
	// Generate the mobile bindings for gdubx and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.dubxcoin", "github.com/DUBXCOIN/go-dubxcoin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gdubx.aar"), "gdubx.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "dubxcoin", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
